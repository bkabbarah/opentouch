...
% sit in % comments beside each value. Remaining \TODO markers flag
% claims that are wrong or unverifiable, not missing numbers.
% =====================================================================
...
Each pose sequence is a window of $T=20$ frames of MANO keypoints,
\TODO{``MANO keypoints'' is wrong --- OpenTouch pose is glove-derived,
21-keypoint MediaPipe-style layout, not MANO. Suggest ``hand keypoints''.}
...
Prior versions of this encoder average-pooled the per-frame features before
projection, discarding the temporal ordering of the window. We instead encode
...
as a $240$-dimensional summary, and apply a single linear projection to the
...
$64$-dimensional shared space. No nonlinearity is applied between the pooled
summary and the projection.
\TODO{previous sentence is FALSE for the GRU arm: a ReLU is applied to the
240-d summary before projection (pose\_encoder.py:108). It is true only for
the avg-pool baseline (pose\_encoder.py:115). Flagged, not rewritten.}
...
% (the 2026_07_06-15_2x pair scores ~72 on the clip gallery -- different
% partition, contaminated, excluded; see verification report)
...
checkpoint scores $10.25$ rather than $16.76$.
\TODO{10.25-vs-16.76: documented at HANDOFF.md:315 and VALIDATION.md:681 but
NO committed eval artifact exists, and regenerating requires the pre-fix
encoder (commit f147054). Either regenerate or soften to ``roughly 10''.
The 16.76 itself is traceable:
results/results\_retrieval\_clip\_avgpool\_test.json .tactile\_to\_pose\_mAP
= 0.167565 --- but note that run's training provenance is unidentified
(HANDOFF \S2.20 caveat); the seeded baselines above supersede it.}
...
\TODO{last sentence is overbroad: bootstrap\_eval.py records
split\_group\_by (+ source) in its output, but the point-estimate eval
(opentouch\_train/eval.py:282-284) historically saved metrics only. The
per-seed JSONs committed for this section carry an injected \_meta block;
older results\_retrieval\_*.json record nothing. Scope the claim to the
bootstrap harness or reword.}
