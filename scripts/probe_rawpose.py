...
parser.add_argument("--random-tactile-encoder", action="store_true",
                    help="Replace the pretrained tactile encoder with a RANDOMLY "
                         "INITIALISED frozen one. The control for whether the +0.039 "
                         "over raw kinematics reflects the LEARNED representation or "
                         "raw tactile signal through a fixed projection. On the probe "
                         "(HANDOFF 2.19e) and forecasting (2.19d) sides the random "
                         "encoder recovered ~71% of the effect.")
...
if args.random_tactile_encoder:
    ...
    print("RANDOM-TACTILE CONTROL: tactile encoder randomly initialised and frozen. "
          "The raw-kinematics baseline is unchanged; only the learned tactile "
          "representation is removed.", flush=True)
else:
